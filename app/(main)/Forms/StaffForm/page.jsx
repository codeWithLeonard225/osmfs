...
import { FiUpload, FiUser, FiEdit2, FiTrash2 } from 'react-icons/fi';
import { HiOutlineStatusOnline, HiOutlineStatusOffline } from 'react-icons/hi';

// Update this path to match your actual Firebase config location
import { db } from '@/app/lib/firebase'; 
// If cloudinaryConfig is in your firebase file, keep this, 
// otherwise move it to an env-based config
import { cloudinaryConfig, uploadToCloudinary } from "@/app/lib/cloudinaryUpload";
...
const Input = ({ id, label, type = 'text', value, onChange, placeholder, readOnly = false }) => (
    <div className="flex flex-col space-y-1">
        ...
        <input
            ...
            className={`w-full p-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-colors duration-200 ${readOnly ? 'bg-gray-100' : 'bg-white text-black'}`}
        />
    </div>
);
...
export default function StaffForm() {
    ...
    // --- EFFECT: Network Status (Next.js Client-Only) ---
    useEffect(() => {
    ...
    // --- EFFECT: Retrieve Session Data (Safely for Next.js) ---
    useEffect(() => {
    ...
    if (file) {
        ...
        try {
            // Use the helper function you created in lib
    ...
    return (
        <div className="container mx-auto p-6 bg-gray-100 min-h-screen font-sans text-gray-900">
            <div className="bg-white rounded-xl shadow-lg p-8 mb-8">
                <div className="flex items-center justify-between border-b pb-4 mb-6">
                    ...
                    <div className="flex items-center gap-2">
                         {isOnline ? <HiOutlineStatusOnline className="text-green-600" /> : <HiOutlineStatusOffline className="text-red-600" />}
                ...
                <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    <div className="flex flex-col space-y-1">
                        ...
                        <select value={gender} onChange={(e) => setGender(e.target.value)} className="p-2 border border-gray-300 rounded-md bg-white text-black">
                    ...
                    <div className="col-span-full flex flex-col items-center py-4 border-t mt-4">
                        <div className="w-24 h-24 rounded-full border-2 border-indigo-200 bg-gray-50 overflow-hidden mb-3 flex items-center justify-center">
                            {imageUploading ? <Spinner /> : imagePreviewUrl ? <img src={imagePreviewUrl} alt="Preview" className="w-full h-full object-cover" /> : <FiUser size={40} className="text-gray-300" />}
                        </div>
                        <Button onClick={() => fileInputRef.current.click()} disabled={imageUploading}>
                            <FiUpload size={16} className="mr-2" /> {imageUploading ? 'Uploading...' : 'Import Photo'}
                        </Button>
                        <input type="file" ref={fileInputRef} onChange={handlePhotoImport} className="hidden" accept="image/*" />
                    </div>

                    <div className="col-span-full">
                        <button type="submit" className="w-full py-3 bg-indigo-600 text-white rounded-md font-bold text-lg hover:bg-indigo-700 shadow-lg transition-all">
            ...
            <div className="bg-white rounded-xl shadow-lg p-8">
                <h2 className="text-2xl font-bold mb-4 text-gray-800">Registered Staff</h2>
                <input 
                    className="mb-4 p-2 border rounded w-full md:w-1/3 text-black" 
                    ...
                    <div className="overflow-x-auto">
                        <table className="min-w-full divide-y divide-gray-200">
                            <thead className="bg-gray-50">
                                <tr>
                                    <th className="px-6 py-3 text-left text-xs font-bold uppercase text-gray-500">Staff ID</th>
                                    <th className="px-6 py-3 text-left text-xs font-bold uppercase text-gray-500">Name</th>
                                    <th className="px-6 py-3 text-left text-xs font-bold uppercase text-gray-500">Gender</th>
                                    <th className="px-6 py-3 text-left text-xs font-bold uppercase text-gray-500">Actions</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-gray-200 text-gray-700">
                                {filteredStaff.map((staff) => (
                                    <tr key={staff.id} className="hover:bg-gray-50 transition-colors">
                                        <td className="px-6 py-4 font-medium text-indigo-600">{staff.staffId}</td>
                                        <td className="px-6 py-4">{staff.fullName}</td>
                                        <td className="px-6 py-4">{staff.gender}</td>
                                        <td className="px-6 py-4 flex space-x-4">
                                            <button onClick={() => handleEdit(staff)} className="text-blue-600 hover:text-blue-800"><FiEdit2 size={18} /></button>
                                            <button onClick={() => handleDelete(staff.id)} className="text-red-600 hover:text-red-800"><FiTrash2 size={18} /></button>
                                        </td>
                                    </tr>
                                ))}
                            </tbody>
